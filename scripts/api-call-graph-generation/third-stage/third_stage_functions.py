...
def api_cg_creation(apk_name, apk_group, selected_api_dict, json_ouput_dir, first_stage_dir):
    try:
        api_cg_path = os.path.join(json_ouput_dir, apk_group, apk_name + '.json')
        ensure_directories_exist([os.path.dirname(api_cg_path)])

        # if file already exists, skip
        if os.path.exists(api_cg_path):
            logging.info(f"API call graph for {apk_name} already exists. Skipping...")
            return
        
        start_time = time.time()

        android_api_dir = "android_apis"
        custom_method_dir = "custom_methods"
        df_normalized_dir = "df_normalized_csv"

        logging.info(f"Generating graph of {apk_name}")

        # Get Android APIs used in the APK
        android_api_path = os.path.join(first_stage_dir, android_api_dir, apk_group, apk_name + '.txt')
        # Get custom methods used in the apk
        custom_method_path = os.path.join(first_stage_dir, custom_method_dir, apk_group, apk_name + '.csv')
        
        with open(android_api_path, 'r') as android_api_file, open(custom_method_path, 'r', encoding='utf-8') as custom_method_file:
            android_apis = set(android_api_file.read().splitlines())
            custom_methods = set(custom_method_file.read().splitlines())

        # Get selected Android APIs used in the APK
        dangerous_android_apis = android_apis.intersection(selected_api_dict)

        # Get df_normalized
        df_normalized_path = os.path.join(first_stage_dir, df_normalized_dir, apk_group, apk_name + '.csv')
        df_normalized = pd.read_csv(df_normalized_path)
        df_normalized['opcode_output'] = df_normalized['opcode_output'].apply(deserialize_opcode_output)
        ...
        if apk_group == "benignware":
            apk_type = 0
        elif apk_group == "malware":
            apk_type = 1
        else:
            raise ValueError("Invalid APK group")

        get_api_call_graph(apk_type, graph, selected_api_dict, dangerous_android_apis, api_cg_path)
        
        end_time = time.time()
        elapsed_time = end_time - start_time
        logging.info("Generated graph for %s in %.2f seconds: %s Android API and %s Custom Methods are used, ", 
                    apk_name, elapsed_time, len(android_apis), len(custom_methods))

    except Exception as e:
        logging.error(f"Exception in API call graph creation of {apk_name}: {e}, Detailed traceback: {traceback.format_exc()}")
        raise e
...
def generate_cfg(df, custom_method_set, android_api_set):
    try:
        ...
        logging.error(f"Exception in generating control flow graph: {e}")
        raise e
...
def get_api_call_graph(apk_type, call_graph, given_api_dict, given_api_set, json_file):
    try:
        ...
        logging.error(f"Exception in getting API call graph: {e}")
        raise e
